            ...
            ghtmlSrc({                 
                ...
                getFileName:function(node){
                    ...
                    if(url){
                        ...
                        if(url.indexOf("_common") >= 0){
                            url = url.replace("../_common", "dev/_common")
                            cb(url)        
                        }else if(url.indexOf("_images") >= 0){
                            url = "dev/"+projectName+"/"+url
                            cb(url)        
                        }
...
function log_free(projectName){  
    ...
    var stream =  gulp.src(entry)
        ...
        .pipe(replace("../_common/images/"+size+"/v2/", ''))
        .pipe(replace("../_common/images/"+size+"/v1/", ''))
        .pipe(replace("../_common/images/"+size+"/", ''))
        .pipe(replace("_images/", ''))
        .pipe(replace(name+"/", ''))
